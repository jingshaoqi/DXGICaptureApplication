...
#undef CHECK_HR
#define CHECK_HR(x) if (FAILED(x)) { fprintf(stdout, "Operation Failed line:%d", __LINE__); goto bail; }
...
void modify_pixels(IMFSample *pSample)
{
	HRESULT hr = S_OK;
	DWORD nLength = 0;
	IMFMediaBuffer *pMediaBuffer = NULL;
	CHECK_HR(hr = pSample->GetBufferByIndex(0, &pMediaBuffer));
	//CHECK_HR(hr = pSample->ConvertToContiguousBuffer(&pMediaBuffer));
	CHECK_HR(hr = pMediaBuffer->GetMaxLength(&nLength));
	//CHECK_HR(hr = pMediaBuffer->GetCurrentLength(&nLength));

	LONGLONG phnsSampleDuration = 0;
	pSample->GetSampleDuration(&phnsSampleDuration);

	printf("video size equal:%s %u\n", VIDEO_BUFFER_SIZE == nLength ? "true" : "false", nLength);
	...
	if (nLength > 0)
	{
		...
		static BYTE pix = 0;
		memset(pBuffer, pix-30, nLength / 3);
		memset(pBuffer+ nLength / 3, pix, nLength/3);
		memset(pBuffer+ nLength*2 / 3, pix+30, nLength/3);
		pix += 4;
		pMediaBuffer->Unlock();
...
void main(int argc, char *argv[])
{
	...
	pf = fopen("test.wmv", "wb");
	
	for(INT i = 0; i < 400; ++i)
	{
		CHECK_HR(hr = pSampleIn->SetSampleTime(rtStart));
		modify_pixels(pSampleIn);
